...
class VDecryptionShareRecord implements VRecord {
  ...
  verify(recorder: VRecorder): arithm.ModPGroupElement | Error {
    ...
    if (isError(shareElement)) {
      ...
      const chaumPedersen = new VChaumPedersenProofRecord(
        this.context,
        this.label,
        this.share.proof,
        this.publicKey,
...
export class VDecryptionRecord implements VRecord {
  ...
  verify(recorder: VRecorder): void {
    if (this.publicKeys.length == 0) {
      recorder.record(
        ...
        this.context,
        'PublicKeysLoading',
        "The public keys couldn't be loaded"
      )
    } else {
      ...
      const alpha = strDecToModPGroupElement(encrypted.public_key, group)
      const beta = strDecToModPGroupElement(encrypted.ciphertext, group)
      const gMessage = strDecToModPGroupElement(decrypted, group)
      ...
      if (isError(alpha)) {
        ...
        if (this.allShares(result)) {
          ...
          if (!isError(beta) && !isError(gMessage)) {
            ...
            const clearElement = strDecToPRingElement(cleartext, group.pRing)
            ...
            const error: Error = firstError([beta, gMessage])
            recorder.record(
              false,
              this.context,
              'DecryptionData',
              'Error loading beta and decrypted tally ' + ': ' + error.message
            )
          }
        } else {
          recorder.record(
  ...
  private isTallyDecryption(
    value: TallyDecryption | SpoiledDecryption
