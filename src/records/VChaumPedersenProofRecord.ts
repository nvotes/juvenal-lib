...
export class VChaumPedersenProofRecord implements VRecord {
  ...
  verify(recorder: VRecorder): void {
    ...
    try {
      ...
      const commitment1 = strDecToByteTree(this.proof.commitment.public_key)
      const commitment2 = strDecToByteTree(this.proof.commitment.ciphertext)

      if (!isError(commitment1) && !isError(commitment2)) {
        const instancePair = new eio.ByteTree([AbyteTree, BbyteTree])
        const commitmentPair = new eio.ByteTree([commitment1, commitment2])
        ...
        recorder.record(
          ...
          this.context,
          'ChaumPedersenProof',
          'The Chaum-Pedersen proof of ' + this.proofTitle + ' should verify'
        )
      } else {
        const error: Error = firstError([commitment1, commitment2])
        recorder.record(
          false,
          this.context,
          'ChaumPedersenProof',
